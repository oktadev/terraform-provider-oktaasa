		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccProjectCheckExists2("asa_project.test", project),
					...
					resource.TestCheckResourceAttr(
						"asa_enrollment_token.test-token", "project_name", projectName,
					),
					resource.TestCheckResourceAttr(
						"asa_enrollment_token.test-token", "description", "Token for TestAcc",
					),
				),
			},
			//Note: ASA does not allow token or token description changes once created (hence there is no Update step)
		},
	})
}
